...
function parseInput() {
    ...
    if (!userInput) {
        updateGameOutput("You are unable to do that.");
        return;
    }
    ...
    if (roomData.commands[userInput]) {
        ...
        if (userInput === 'look' || userInput.includes('examine')) {
            if (!gameState.inventory.includes('glasses')) {
                updateGameOutput('You can not see clearly without your <span style="color: white; font-weight: bold;">GLASSES</span>. <i>It must have fallen to the</i> <span style="color: white; font-weight: bold;">FLOOR</span> <i>somewhere...</i>', userInput);
            }
            else {
            ...
            if (gameState.action.includes('call_1')) {
                updateGameOutput('The telephone is no longer ringing.', userInput);
            }
            else {
                ...
                gameData.start.description = 'You\'re in a dark room. There\'s a staircase leading down <i>south</i> to the shop on the first floor. The bedrooms are to the <i>east</i> while the dining room and kitchen are to the <i>west</i> of here. The room is silent now.';
                updateGameOutput(command.output, userInput);
                startDialogueScene();
            }
        }
